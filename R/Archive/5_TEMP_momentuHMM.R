...
PlotLocationSunriseSunset(
  df=baea_behavior %>% as.data.frame() %>% filter(id == "Wilson"),
  by = "id", color_factor = "behavior",
  individual = "", start = "2015-06-15", end = "2015-09-15", breaks = "10 days", tz = "Etc/GMT+5",
...
for (i in unique(baea_behavior$behavior)){
  ...
  vm_pars <- mledist(baea_behavior_von_mises_i$turn_angle, "vm",
    weights = round(baea_behavior_von_mises_i$weights*10000),
...
saveRDS(baea_hmm_start, file = "Data/Models/baea_hmm_start2")
...
saveRDS(baea_hmm_full, file = "Data/Models/baea_hmm_full2")
...
plot(baea_hmm_full)
P <- momentuHMM::allProbs(baea_hmm_full, nbStates = n_states)
P
