...
sim$pars$global$sim_end <- as.POSIXct("2015-09-20", tz = "UTC")
...
RunSimulation <- function(sim = sim,
  ...
  for (i in 1:length(runs)){
    ...
    #saveRDS(sim, file="Data/Simulation/sim_stepsTEST.RDS")
    #sim <- readRDS("Data/Simulation/sim_stepsTEST.RDS")
...
sim_step_data$behavior <- c("Cruise", "Flight", "Nest", "Perch","Roost")

sim_step_data <- ConvertStepDataCoordinates(sim_step_data)
output_dir = "Output/Plots/Behavior/Sim"
...
for (i in unique(sim_step_data$id)){
  ...
  ExportKMLTelemetry(sim_step_data_i, lat="lat", long="long", alt=NULL,
    speed=NULL, file = paste0("Sim_", i, ".kmz"), icon_by_sex=TRUE,
    behavior = "behavior", point_color = "behavior", output_dir=output_dir)
}
