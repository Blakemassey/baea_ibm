...
## Import Baea and Baea_movements ----------------------------------------------
...
move_pars <- readRDS("Results/Analysis/Tables/move_pars.RDS")

baea_steps <- left_join(baea_movements, baea_locs, by = c("id", "datetime"))
rm(baea, baea_movements, baea_locs)

# Step type
step_type <- "Flight -> Flight"

# Create Movement Kernel
move_pars_i <- move_pars %>% filter(behavior_behavior == step_type)
...
plot(prob_raster)
plot(RotateRaster(prob_raster, 135))
...
lc_crop <- crop(lc_30mc, extend(extent(baea_steps_type_sp), 500))


for (i in 1:nrow(baea_steps_type_sp)){
  exp_angle_i <- baea_steps_type_sp[i, "exp_angle"]
  print(exp_angle_i)
  }
