################ This script converts SSF Models to SSF Surfaces #############-#
...
best_ssf_fits_org <- readRDS(file.path(mod_fit_dir,
  "glm_model_fits_compiled_best",
  "glm_models_compiled_best.rds"))
...
saveRDS(preds_tbl, file.path(mod_fit_dir, "glm_model_fits_compiled_best",
  "preds_tbl.rds"))

################ CREATE COVARIATE RASTERS FOR MAINE ############################

preds_tbl <- readRDS(file.path(mod_fit_dir, "glm_model_fits_compiled_best",
  "preds_tbl.rds"))
...
for (i in 1:nrow(preds_kernel)){
  ...
  covar <- preds_kernel_i %>% pull(covar)
    print(paste0("Checking: ", covar, sigma, " (", i, " of ",
  ...
  if(!file.exists(out_file)){
    ...
    print(paste0("Starting: ", covar, sigma, " at: ", lubridate::now()))
    if(sigma == 0){
...
preds_tbl_file = file.path(mod_fit_dir, "glm_model_fits_compiled_best",
  "preds_tbl.rds")
...
step_type_dir = file.path(ssf_raster_dir, "Step_Types_GLM")
maine_raster_trim_file = "C:/ArcGIS/Data/BlankRaster/maine_trim.tif"

best_ssf_fits <- readRDS(file.path(mod_fit_dir,
...
for (i in 1:nrow(best_ssf_fits)){
  ...
  terms_i <- best_ssf_fits %>% slice(i) %>% pluck("fit_terms", 1) %>%
    .[!. %in% c("strata(step_id)")]
  # Extract coefficients
  coefs_i <- best_ssf_fits %>% slice(i) %>% pluck("fit_coefs", 1)
  # Create Raster_Brick
  ...
  writeRaster(ssf_prob_raster, file.path(ssf_raster_dir,
    "Step_Types_GLM_Prob", step_type_i_numeric), format = "GTiff",
    overwrite = TRUE)
