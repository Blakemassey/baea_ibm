...
con_nest_map_fig <- backgrd %>%
  ...
  image_composite(., legend_title, offset = "+1765+650") %>%
  image_composite(., legend_scale, offset = "+1865+960") %>%
  image_flatten(.)
con_nest_map_fig
...
for (i in seq_len(length(ssf_prob_files))){
  ...
  if(end_behavior == "Perch"){
    step_type_bottom <- expression(Perch)
  }
  if(end_behavior == "Roost"){
  ...
  ssf_prob_i_map <-
    tm_shape(ssf_prob_i, raster.downsample = FALSE) +
      ...
      tm_layout(asp = .8,
        ...
        legend.text.fontfamily = "Latin Modern Roman",
        frame = FALSE) +
      tm_credits(step_type_top, size = 0.85, fontface = "bold",
        position = c(.0, .87)) +
      tm_credits(step_type_bottom, size = 0.85, fontface = "bold",
        position = c(.025, .75))
...
legend_title <- legend_a_img %>% image_crop(., "300x50+50+0")
legend_a_title <- legend_a_img %>% image_crop(., "50x50+0+0")
legend_a_title
legend_a_scale <- legend_a_img %>% image_crop(., "150x165+0+70")# %>%
legend_a_scale

legend_b_img <- legend_b_file %>% image_read(.) %>% image_trim(.)
legend_b_title <- legend_b_img %>% image_crop(., "50x50+0+0")
legend_b_scale <- legend_b_img %>% image_crop(., "150x155+0+70")

legend_c_img <- legend_c_file %>% image_read(.) %>% image_trim(.)
legend_c_title <- legend_c_img %>% image_crop(., "50x50+0+0")
...
ssf_maps_fig <- image_blank(1850, 2395, color = "white") %>%
  ...
  image_composite(., legend_title,   offset = "+1470+1935") %>%
  image_composite(., legend_a_title, offset = "+1390+2000") %>%
  image_composite(., legend_a_scale, offset = "+1440+2040") %>%
  image_composite(., legend_b_title, offset = "+1620+2000") %>%
  image_composite(., legend_b_scale, offset = "+1670+2040") %>%
  image_composite(., legend_c_title, offset = "+1390+2210") %>%
  image_composite(., legend_c_scale, offset = "+1440+2250")
ssf_maps_fig
  ...
  ssf_prob_i_map <-
    tm_shape(ssf_prob_i, raster.downsample = FALSE) +
      ...
      tm_layout(asp = .8,
        ...
        legend.text.fontfamily = "Latin Modern Roman",
        frame = FALSE) +
      tm_credits(step_type_top, size = 1, fontface = "bold",
        position = c(.0, .85)) +
      tm_credits(step_type_bottom, size = 1, fontface = "bold",
        position = c(.025, .72))
...
# BE AWARE: Legend figures should come from the 'SSF for Maine - Combined' code
map_img <- maps_file %>% image_read(.) %>% image_trim(.)
legend_a_img <- legend_a_file %>% image_read(.) %>% image_trim(.)
legend_title <- legend_a_img %>% image_crop(., "250x75+70+00") %>%
  image_resize(., "325x")
legend_title
legend_a_title <- legend_a_img %>% image_crop(., "55x50+0+0") %>%
  image_resize(., "70x")
legend_a_title
legend_a_scale <- legend_a_img %>% image_crop(., "150x165+0+70") %>%
  image_resize(., "200x")
...
legend_b_img <- legend_b_file %>% image_read(.) %>% image_trim(.)
legend_b_title <- legend_b_img %>% image_crop(., "55x50+0+0") %>%
  image_resize(., "70x")
legend_b_scale <- legend_b_img %>% image_crop(., "150x155+0+70") %>%
  image_resize(., "200x")

legend_c_img <- legend_c_file %>% image_read(.) %>% image_trim(.)
legend_c_title <- legend_c_img %>% image_crop(., "55x50+0+0") %>%
  image_resize(., "70x")
legend_c_scale <- legend_c_img %>% image_crop(., "150x155+0+70") %>%
  image_resize(., "200x")

ssf_maps_fig <- image_blank(2800, 2395, color = "white") %>%
  image_composite(., map_img, offset = "+0+0") %>%
  image_composite(., legend_title,   offset = "+2400+700") %>%
  image_composite(., legend_a_title, offset = "+2400+800") %>%
  image_composite(., legend_a_scale, offset = "+2500+800") %>%
  image_composite(., legend_b_title, offset = "+2400+1050") %>%
  image_composite(., legend_b_scale, offset = "+2500+1050") %>%
  image_composite(., legend_c_title, offset = "+2400+1300") %>%
  image_composite(., legend_c_scale, offset = "+2500+1300")
ssf_maps_fig

# Export
ssf_maps_fig_file <- file.path("C:/Users/blake/OneDrive/Work/R/Projects",
  "dissertation_presentation/Assets/Images/Ch2_Extra",
  "SSF_Probability_Maps_Overview.png")
image_write(ssf_maps_fig, path = ssf_maps_fig_file, format = ".png")
...
for (j in seq_len(nrow(nests_sim))){
  ...
  ssf_maps_fig <- image_blank(1900, 2400, color = "white") %>%
    image_composite(., map_img, offset = "+0+0") %>%
    image_composite(., legend_title,   offset = "+1520+1915") %>%
    image_composite(., legend_a_title, offset = "+1450+1980") %>%
    image_composite(., legend_a_scale, offset = "+1500+2020") %>%
    image_composite(., legend_b_title, offset = "+1680+1980") %>%
    image_composite(., legend_b_scale, offset = "+1730+2020") %>%
    image_composite(., legend_c_title, offset = "+1450+2185") %>%
    image_composite(., legend_c_scale, offset = "+1500+2230")
  ssf_maps_fig
...
for (i in seq_len(nrow(ssf_fits_best))){
  ...
  ssf_prob_i_nest_map <-
    ...
    tm_scale_bar(breaks = c(0, 5, 10), text.size = .8, lwd = .25,
      position = c(.11, -.04)) +
    tm_compass(type = "4star", text.size = 0.9, show.labels = 1,
      size = 1.6, position = c(.69, .645), lwd = .25) +
    tm_shape(nests_sim) +
    tm_symbols(shape = 21, border.col = "black", border.lwd = 1,
      col = nest_color, size = .15) +
    tm_layout(asp = .8,
      fontfamily = "Latin Modern Roman",
      frame = NA,
      title.color = "black",
    ...
    tm_credits(legend_label, bg.color = "gainsboro",
      position = c(.825, .05),
      just = "center", size = 0.75, width = .10)
...
ssf_maps_fig <- image_blank(2800, 2400, color = "white") %>%
  image_composite(., map_img, offset = "+0+0") %>%
  image_composite(., legend_title,   offset = "+2400+700") %>%
  image_composite(., legend_a_title, offset = "+2400+800") %>%
  image_composite(., legend_a_scale, offset = "+2500+800") %>%
...
# Export
ssf_nest_map_fig_file <- file.path("C:/Users/blake/OneDrive/Work/R/Projects",
  "dissertation_presentation/Assets/Images/Ch2_Extra",
  paste0("SSF_Probability_Maps_", nest_j_name, ".png"))
...
# Conspecific and Home Nest Raster Maps ----------------------------------------
...
if(FALSE) mapview::mapview(con_nest_crop)
...
if(FALSE) xy_con_nest <- 5000 # For testing
con_nest_centered <- raster::calc(con_nest_crop,
  fun = function(x){(x - xy_con_nest)/1000})
if(FALSE) mapview::mapview(con_nest_centered)
...
if(FALSE) mapview::mapview(con_nest_centered_mask)
...
if(FALSE) mapview::mapview(con_nest_prob)
...
legend_title
legend_scale <- legend_img %>% image_crop(., "250x550+0+65")
legend_scale
...
kernel_map_fig <- backgrd %>%
  ...
  image_composite(., legend_scale, offset = "+1800+730") %>%
  image_flatten(.)
kernel_map_fig
...
# Map of Recentered Distance Map (not used)
recentered_map <-
  tm_shape(kernel_om) +
  tm_rgb() +
...
for (j in unique(sim_step_data$baea_id)){
  ...
  density_ridges_map <-
    tm_shape(nests_sim) +
    tm_symbols(shape = 21, border.col = "black", border.lwd = 2,
      col = nest_color, size = .75) + # FOR DISSERTATION use size = .25
  ...
  legend_scale_height <- image_info(legend_scale_img) %>%
    pull(height)
  legend_scale_offset <- 350

  legend_title_height <- image_info(legend_title_img) %>%
    pull(height)
  legend_title_offset <- 250

  density_presentation_fig <- backgrd %>%
    image_composite(., baea_map_img, offset = "+0+10") %>%
    image_composite(., sim_map_img, offset = "+755+10") %>%
    image_composite(., legend_title_img, offset = paste0("+1520+",
      legend_title_offset)) %>%
    image_composite(., legend_scale_img, offset = paste0("+1560+",
      legend_scale_offset))
  density_presentation_fig

  # Export PNG
  image_write(density_presentation_fig, path = file.path("C:/Users",
    "blake/OneDrive/Work/R/Projects/dissertation_presentation/Assets/Images",
    "Ch3_Extra", paste0(j, "_BAEA_Sim_Density_Map.svg")), format = "svg",
    flatten = TRUE)
...
for (i in c("Grand_Lake", "Wilson")){
  ...
  exp_scenarios <- list.dirs(file.path(gis_exp_dir, nest_str),
    recursive = FALSE)
  ...
  tmap_save(tm = tmap_nest_wind_areas, filename = file.path(tex_dir,
    ...
    insets_tm = nest_overview, insets_vp =  viewport(x = 0.859, y = .141,
    ...
    for (i in seq_len(nrow(ssf_fits_best))){
      ...
      if (i ==  1){
        ...
        nest_bb_sf <- st_as_sfc(bb(nest_buffer, relative = TRUE, height = 1.35,
          width = 1.35))
        Sys.sleep(1)
        nest_om = read_osm(nest_bb_sf, type = om_nat_geo, zoom = 11) #minNumTiles=9
        nest_om_bb <- bb_poly(nest_om)
      }
      ssf_prob_i <- raster(ssf_prob_file) #%>% slice(1)
      ssf_prob_i_crop <- crop(ssf_prob_i, nest_buffer)
  ...
  for (j in c("Cruise", "Flight")){
    ...
    if(j == "Cruise") background <- image_blank(1883, 2100, color = "white")
    if(j == "Flight") background <- image_blank(1883, 2150, color = "white")

    path_density_fig <- background %>%
      image_composite(., paths_density_map_img, offset = "+0+0") %>%
      image_composite(., legend_img, offset = "+1450+1790")
    path_density_fig
  ...
  for (j in unique(baea_hr_i$year)){
    ...
    baea_k_hr_paths <-
      ...
      tm_scale_bar(text.size = 1.25,
        ...
        position = c(.05, .01)) +
      tm_compass(type = "4star",  show.labels = 1, size = 2.25, text.size = 1.25,
        position = c(.85, .85)) +
      tm_xlab("") + tm_ylab("")
  ...
  for (i in seq_len(length(ssf_prob_files))){
    ...
    ssf_prob_i_map <-
      ...
      tm_layout(#asp = .75,
        ...
        frame = FALSE)
    ssf_prob_i_map
    tmap_save(tm = ssf_prob_i_map, filename = file.path(tex_dir,
      "Figures/Appendix", "SSF_Prob_Raster_Maps", paste0("SSF_Probability_Map_",
        step_type_text, ".png")), unit = "in", dpi = 300, height = 8, width = 6)
    }
}
######################## OLD CODE ##############################################
