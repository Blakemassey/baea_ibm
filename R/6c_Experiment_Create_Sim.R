...
baea_wilson <- readRDS("Data/BAEA/baea_wilson.rds")
base <- raster("C:/ArcGIS/Data/BlankRaster/maine_30mc.tif")

nests_agents <- baea_wilson %>%
  group_by(id) %>%
  slice(1) %>%
...
#RemoveExcept("sim")
saveRDS(sim, file = "C:/Work/Sim_Data/sim_20210620_Wilson.rds")

# Recompile sim
sim <- readRDS(file = "C:/Work/Sim_Data/sim_20210620_Wilson.rds")
agents <- sim$agents
pars <- sim$pars
spatial <- sim$spatial
