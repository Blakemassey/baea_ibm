...
baea_movements <- baea_behavior %>%
  ...
  mutate(step_time2 = as.integer(lead(datetime) - datetime)) %>%
  mutate(end_lat_utm = lead(lat_utm)) %>%
  mutate(end_long_utm = lead(long_utm)) %>%
  #filter(step_time2 <= 20)  %>%
  #filter(step_length > 42.43) %>%
  #filter(!is.na(turn_angle)) %>%
  mutate(move = ifelse(step_time2 <= 20 & step_length > 42.43 &
    !is.na(turn_angle), TRUE, FALSE))  %>%
  ungroup() %>%
  mutate(behavior_behavior = paste(behavior, "->", behavior_next)) %>%
  dplyr::select(id, datetime, step_time, step_time2, move, speed, alt, agl, dx,
    dy, step_length, behavior, behavior_next, behavior_behavior, turn_angle,
    lat_utm, long_utm, end_lat_utm, end_long_utm)

baea_movements_all <- baea_movements %>%
  filter(move == TRUE)

baea_movements_pc <- baea_movements_all %>%
  filter(behavior_behavior == "Perch -> Roost")

baea_movements_pp <- baea_movements_all %>%
