...
# Set to NA for "model_best_fits.rds"
...
# Model files
model_file <- ifelse(is.na("model_id"), "model_fits_best.rds",
  paste0("model_fits_best_", model_id, ".rds"))
fits_best_file <- file.path(mod_best_dir, model_file)
preds_file <- ifelse(is.na("model_id"), "preds_tbl.rds",
  paste0("preds_tble_", model_id, ".rds"))
preds_tbl_file <- file.path(mod_best_dir, preds_file)

# SSF directories
ssf_raster_dir <- "C:/ArcGIS/Data/R_Input/BAEA/SSF_Rasters"
ssf_prob_model_dir <- file.path(ssf_raster_dir, "Step_Types_Prob_Model_ID")
ssf_prob_model_id_dir <- file.path(ssf_prob_model_dir, model_id)
...
if(!dir.exists(ssf_prob_model_id_dir)){
...
ViewProbsNestMap <- function(nest_name,
  ...
  tmap_i <-
    ...
    tm_raster(alpha = opacity, palette = "viridis", legend.reverse = TRUE,
      n = 10, title = paste0(step_types_arrow, "<br>Probability")) +
...
step_type <- ssf_fits_best %>% slice(5) %>% pull(step_type)
