...
# Statistical tests
wind_crossings_sum_test_north_area <- wind_crossings_sum %>%
  group_by(behavior_line) %>%
  t_test(n_area_prop ~ scenario) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")

wind_crossings_sum_test_south_area <- wind_crossings_sum %>%
  group_by(behavior_line) %>%
  t_test(s_area_prop ~ scenario) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")

wind_crossings_sum_test_north_turbines <- wind_crossings_sum %>%
  group_by(behavior_line) %>%
  t_test(n_turbines_prop ~ scenario) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")

wind_crossings_sum_test_south_turbines <- wind_crossings_sum %>%
  group_by(behavior_line) %>%
  t_test(s_turbines_prop ~ scenario) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")

datatable(wind_crossings_sum_test_north_area,
  caption = "North Area Crossings",
  options = list(
  searching = FALSE,
  pageLength = 12,
  dom = 'ft'))

datatable(wind_crossings_sum_test_south_area,
  caption = "South Area Crossings",
  options = list(
  searching = FALSE,
  pageLength = 12,
  dom = 'ft'))

datatable(wind_crossings_sum_test_north_turbines,
  caption = "North Turbines Crossings",
  options = list(
  searching = FALSE,
  pageLength = 12,
  dom = 'ft'))

datatable(wind_crossings_sum_test_south_turbines,
  caption = "South Turbines Crossings",
  options = list(
  searching = FALSE,
  pageLength = 12,
  dom = 'ft'))
...
```{r echo=F, fig.height=5.2, fig.width=9, message=FALSE, warning=FALSE}
...
theme_crossings <- theme(
  ...
  theme(plot.margin = margin(.85,1,.85,1, "cm"))  
