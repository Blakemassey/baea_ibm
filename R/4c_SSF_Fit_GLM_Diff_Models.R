...
mod_fit_dir <- "Output/Analysis/SSF/Models/glm_model_fits"
...
#ua_files <- dir(ua_data_dir)[c(5,6)]
...
for (i in seq_along(ua_files)){
  ...
  step_type <- str_remove_all(ua_file_i, ("ua_steps_diff_|.rds"))
  ...
  ua_steps_i <- ua_steps_i_org  %>%
    dplyr::select(c("case"), matches(keep_covars)) %>%
...
# EXTRACT PARAMETERS -----------------------------------------------------------
  ...
  FitGlm <- function(glm_preds, ua_data){
    glm_model_formula = as.formula(glm_preds)
    glm_fit <- glm(glm_model_formula, data = ua_data, family = 'binomial')
    return(glm_fit)
  }

  ExtractGlmCoefs <- function(glm_fit){
    glm_fit_i <- glm_fit %>% pluck(coef)
    return(glm_fit_i)
  }

  ExtractGlmFitTerms <- function(glm_fit){
    terms_i <- glm_fit %>% pluck(terms, attr_getter("term.labels"))
    return(terms_i)
  }
  ...
  ExtractOptFitFormula <- function(opt_fit, covars_scale, pars, covars_fixed){
    if(all(is.na(opt_fit))){
      ...
      opt_fit_formula <- paste0("case ~ -1 + ", preds)
    }
  ...
  tbl_models_fitted_pars <- tbl_models_fitted %>% #slice(7) %>%
    ...
    mutate(glm_preds = pmap_chr(.l = list(opt_fit, covars_scale, pars,
      covars_fixed), .f = ExtractOptFitFormula)) %>%
    mutate(glm_fit = list(NA)) %>%
    mutate(glm_fit_null = list(NA)) %>%
  ...
  for (i in seq_len(nrow(tbl_models_fitted_pars))){
    glm_preds_i <- tbl_models_fitted_pars %>% slice(i) %>%
      pull("glm_preds")
    if(!is.na(glm_preds_i)){
      glm_model_formula <- as.formula(glm_preds_i)
      glm_fit <- glm(glm_model_formula, data = ua_steps_i, family = 'binomial')
      fit_aicc_refit <- AICc(glm_fit)
      fit_coefs <- glm_fit %>% pluck(coef)
      fit_terms <- glm_fit %>% pluck(terms, attr_getter("term.labels"))
      fit_coefs_signif <- signif(fit_coefs, digits = 4)
      tbl_models_fitted_pars$glm_fit[i] <- list(glm_fit)
  ...
  tbl_models_fitted_final <- tbl_models_fitted_pars %>%
    dplyr::select(step_type, model_num, fit_aicc, delta_aicc, model_chr,
      glm_preds, fit_terms, fit_coefs, fit_coefs_signif)
