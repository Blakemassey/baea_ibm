...
#  slice(1:2000) %>%
  ...
  mutate(
    ...
    ID = as.factor(id)) %>%
  dplyr::select(id, ID, datetime, behavior, behavior_num, #julian_date,
    long_utm, lat_utm, sex, #nest_dist, # added nest_dist
...
# NEW FORMULA
hmm_formula <- ~cosinor(time_proportion, period = 1)
...
#  optMethod = "Nelder-Mead",
  ...
  verbose = 2)
toc()
seconds_to_period(4242)
#Took ~1.16 hours on 2021-06-16
...
beta_est2 <- beta_est %>% as.data.frame(.) %>%
  ...
  rename_all(~(str_replace_all(., "[\\.\\.\\.\\.]", " "))) %>%
  rename_all(~(str_replace_all(., "    ", "->"))) %>%
  rename_all(~(str_replace_all(., "1", "Cruise"))) %>%
  rename_all(~(str_replace_all(., "2", "Flight"))) %>%
  rename_all(~(str_replace_all(., "3", "Nest"))) %>%
  rename_all(~(str_replace_all(., "4", "Perch"))) %>%
  rename_all(~(str_replace_all(., "5", "Roost"))) %>%
  rename_all(~(str_replace_all(., "est ", "")))
